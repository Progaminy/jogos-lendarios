alter table public.ludo_rooms
  add column if not exists negotiation_grace_used boolean not null default false;
...
begin
 ...
 insert into public.ludo_room_players(room_id,player_id,seat,color,team,status) values(p_room,p_player,s,col,tm,'active');
...
begin
 ...
 if r.status='negotiating' and r.action_deadline is not null and r.action_deadline<=now() then
   if not r.negotiation_grace_used then
     update public.ludo_rooms set negotiation_grace_used=true,action_deadline=now()+interval '30 seconds',updated_at=now() where id=p_room returning * into r;
     perform public.jl_ludo_event(p_room,null,'negotiation_grace_started',jsonb_build_object('seconds',30));
   else
     raise exception 'O tempo para aceitar estas regras terminou. Aguarde novo convite ou ajuste da sala.';
   end if;
 end if;
...
begin
 ...
 if r.status='negotiating' and r.action_deadline is not null and r.action_deadline<=now() then
   ...
   if r.action_deadline is not null and r.action_deadline<=now() and r.current_player_id is not null then
     ...
     if (select timeout_strikes from public.ludo_room_players where room_id=p_room and player_id=cur)>=limitn then
       update public.ludo_room_players set status='eliminated' where room_id=p_room and player_id=cur;
       perform public.jl_ludo_event(p_room,cur,'player_eliminated',jsonb_build_object('reason','timeouts'));
     end if;
