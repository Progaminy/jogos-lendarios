alter table public.ludo_rooms add column if not exists public_challenge_expires_at timestamptz;
...
begin
  return coalesce((
    select jsonb_agg(jsonb_build_object(
      'room_id',r.id,'code',r.code,'host_id',r.host_id,'host_name',p.name,
      'host_code',public.jl_ludo_display_code(r.host_id),'player_count',r.player_count,
      'joined_count',x.joined_count,'open_slots',greatest(0,r.player_count-x.joined_count),
      'mode',r.mode,'bet_amount',r.bet_amount,'status',r.status,
      'play_location',coalesce(r.rules->>'play_location','online'),
      'dice_count',coalesce((r.rules->>'dice_count')::int,1),
      'expires_at',r.public_challenge_expires_at,'created_at',r.created_at
    ) order by r.updated_at desc)
    from public.ludo_rooms r
    join public.players p on p.id=r.host_id
    cross join lateral (
      select count(*)::int joined_count from public.ludo_room_players rp
    ...
    where r.is_public
      ...
      and r.public_challenge_expires_at is not null
      and r.public_challenge_expires_at>now()
      and not exists(select 1 from public.ludo_room_players mine where mine.room_id=r.id and mine.player_id=me and mine.status<>'left')
  ),'[]'::jsonb);
end;
$$;
...
begin
  ...
  update public.ludo_rooms set public_challenge_expires_at=now()+interval '60 seconds',updated_at=now() where id=p_room;
  perform public.jl_ludo_event(p_room,me,'public_challenge_rebroadcast',jsonb_build_object('expires_in',60));
