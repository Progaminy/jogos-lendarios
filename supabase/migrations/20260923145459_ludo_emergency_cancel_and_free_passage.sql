-- Ludo: emergency admin cancellation + free passage rule
-- 2026-09-23
...
create or replace function public.jl_admin_cancel_all_ludo(p_token text, p_reason text default 'Cancelamento administrativo de emergência')
returns jsonb
language plpgsql
security definer
set search_path to 'public'
...
update public.ludo_rooms
set rules=jsonb_set(rules,'{blockades}','false'::jsonb,true),
    updated_at=now()
where status not in ('finished','cancelled')
  and coalesce((rules->>'blockades')::boolean,false) is distinct from false;
