-- Ludo Lendário
-- Regra financeira comum: aposta mínima de 10 MZN.
-- Comissão do Ludo: 1% do ganho individual, sempre arredondada para cima
-- ao metical inteiro (ex.: 1.6 -> 2 MZN; 2.1 -> 3 MZN).

alter table public.ludo_rooms
  drop constraint if exists ludo_rooms_bet_amount_check;
alter table public.ludo_rooms
  add constraint ludo_rooms_bet_amount_check check (bet_amount >= 10);

alter table public.ludo_waiting_queue
  drop constraint if exists ludo_waiting_queue_bet_amount_check;
alter table public.ludo_waiting_queue
  add constraint ludo_waiting_queue_bet_amount_check check (bet_amount >= 10);
...
begin
  ...
  values(
    public.jl_ludo_room_code(),v_player,p_player_count,p_mode,round(p_bet_amount,2),p_is_public,
    v_rules,'waiting',now()+make_interval(secs=>(v_rules->>'rules_response_seconds')::integer)
  ...
  perform public.jl_ludo_event(v_room.id,v_player,'room_created',jsonb_build_object(
    'code',v_room.code,'bet',v_room.bet_amount,'players',v_room.player_count,'mode',v_room.mode
  ));
...
begin
  ...
  if exists(
    ...
    raise exception 'Você já participa de uma sala ativa.';
  end if;
...
begin
  ...
  perform public.jl_ludo_event(p_room,p_winner,'game_finished',jsonb_build_object(
    'winner_player_id',p_winner,'winner_team',p_team,'pot',r.pot,'commission_total',total_comm
  ));
end;
$$;
