...
begin
  ...
  if r.status='negotiating' and r.action_deadline is not null and r.action_deadline<=now() then
    if not r.negotiation_grace_used then
      ...
      set negotiation_grace_used=true,
          action_deadline=now()+interval '30 seconds',
          updated_at=now()
      where id=p_room;
      ...
      where room_id=p_room
        ...
        and coalesce(accepted_rules_version,0)<>r.rules_version;
      update public.ludo_rooms
      set status='waiting',
    ...
    delete from public.ludo_room_players
    where room_id=p_room and player_id<>r.host_id and not stake_paid;
    perform public.jl_ludo_refund_room(p_room,'Reembolso: tempo de confirmação da sala expirou');
    update public.ludo_room_players
    set accepted_rules_version=case when player_id=r.host_id then r.rules_version else null end
    where room_id=p_room;
    update public.ludo_rooms
    set status='waiting',
        action_deadline=null,
    ...
    if r.action_deadline is not null
      ...
      where room_id=p_room and player_id=cur;
