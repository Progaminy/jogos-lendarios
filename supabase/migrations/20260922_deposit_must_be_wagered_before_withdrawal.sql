-- Melhoria 15: depósito deve ser jogado antes de poder ser sacado.
...
-- Reconstrói depósitos já aprovados e desconta somente apostas que ocorreram depois de cada aprovação.
do $$
declare e record;
begin
  if not exists(select 1 from public.deposit_wager_requirements) then
    for e in
      with events as (
        select d.player_id,coalesce(d.reviewed_at,d.created_at) event_at,0 event_order,
               'deposit'::text event_type,d.id reference_id,d.amount
        from public.deposit_requests d where d.status='approved'
        union all
        select b.player_id,b.created_at,1,'number_bet',b.id,b.cash_amount
        from public.bets b where b.cash_amount>0
        union all
        select pb.player_id,pb.created_at,1,'pair_bet',pb.id,pb.cash_amount
        from public.pair_bets pb where pb.cash_amount>0
        union all
        select t.player_id,t.created_at,1,'ludo_stake',t.reference_id,abs(t.amount)
        from public.transactions t
        where t.kind='ludo_stake' and t.status='completed' and t.reference_id is not null and t.amount<0
        union all
        select t.player_id,t.created_at,1,'ludo_reentry',t.reference_id,abs(t.amount)
        from public.transactions t
        where t.kind='ludo_reentry' and t.status='completed' and t.reference_id is not null and t.amount<0
        union all
        select t.player_id,t.created_at,2,'ludo_refund',t.reference_id,t.amount
        from public.transactions t
        where t.kind='ludo_refund' and t.status='completed' and t.reference_id is not null and t.amount>0
      )
      select * from events order by player_id,event_at,event_order,reference_id
    loop
      if e.event_type='deposit' then
        insert into public.deposit_wager_requirements(
          deposit_request_id,player_id,original_amount,remaining_amount,status,created_at,updated_at
        ) values(e.reference_id,e.player_id,e.amount,e.amount,'active',e.event_at,e.event_at);
      elsif e.event_type='number_bet' then
        perform public.jl_apply_cash_wager(e.player_id,e.amount,'number_bet',e.reference_id);
      elsif e.event_type='pair_bet' then
        perform public.jl_apply_cash_wager(e.player_id,e.amount,'pair_bet',e.reference_id);
      elsif e.event_type='ludo_stake' then
        perform public.jl_apply_cash_wager(e.player_id,e.amount,'ludo_stake',e.reference_id);
      elsif e.event_type='ludo_reentry' then
        perform public.jl_apply_cash_wager(e.player_id,e.amount,'ludo_reentry',e.reference_id);
      elsif e.event_type='ludo_refund' then
        perform public.jl_reverse_cash_wager(e.player_id,'ludo_stake',e.reference_id);
      end if;
    end loop;
  end if;
end $$;
