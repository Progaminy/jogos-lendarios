(() => {
  ...
  const state = {
    ...
    rulesFormDirty: false, rulesFormVersion: null, lastFxEventId: 0, autoMoveKey: null
  };
  ...
  function entryRuleRows(){
    ...
    const rows=[
      ...
      `Saída da base: ${x.base_exit_rule==='one_or_six'?'1 ou 6':'somente 6'}`,
      ...
      boolLabel(x.blockades,'Barreiras ativas','Sem barreiras'),
  ...
  function syncEntryFlowModals(){
    ...
    const needsRules=['waiting','negotiating'].includes(r.status)&&mine.accepted_rules_version!==r.rules_version;
  ...
  els.rulesModalAccept?.addEventListener('click',()=>withBusy(async()=>{try{state.room=await rpc('jl_ludo_accept_rules',{p_token:state.token,p_room:roomData().id,p_accept:true});renderRoom();showToast('Regras aceites.','success');}catch(err){showToast(err.message,'error');}}));
  els.rulesModalDecline?.addEventListener('click',()=>withBusy(async()=>{try{state.room=await rpc('jl_ludo_accept_rules',{p_token:state.token,p_room:roomData().id,p_accept:false});renderRoom();showToast('Você não aceitou esta versão. Negociem outra regra.');}catch(err){showToast(err.message,'error');}}));
  ...
  els.fundButton.addEventListener('click',()=>withBusy(async()=>{try{state.room=await rpc('jl_ludo_commit_stake',{p_token:state.token,p_room:roomData().id});await loadStatus(true);showToast('Aposta confirmada.','success');}catch(err){showToast(err.message,'error');}}));
  els.stakeModalAccept?.addEventListener('click',()=>withBusy(async()=>{try{els.stakeModalAccept.disabled=true;state.room=await rpc('jl_ludo_commit_stake',{p_token:state.token,p_room:roomData().id});await loadStatus(true);showToast('Valor confirmado.','success');}catch(err){showToast(err.message,'error');}finally{els.stakeModalAccept.disabled=false;syncEntryFlowModals();}}));els.rollDice.addEventListener('click',()=>withBusy(async()=>{
    if(state.animating)return;
    els.rollDice.disabled=true;
